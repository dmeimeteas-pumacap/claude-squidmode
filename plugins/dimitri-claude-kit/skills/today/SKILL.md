...
argument-hint: "[set | show | done <n> | undone <n> | add \"<task>\"]"
---
...
2. Read the `goals/modes/today.md` mode file (path resolution above) and follow it. Treat any text
   after `/today` as the trailing argument: `set` (make the day's plan) and `show` (relay it) are
   the two explicit verbs; `done <n>` / `undone <n>` / `add` are its step-5 verbs. **Bare `/today`
   resolves by store state** — `show` when the store's header date is `<DATE>`, `set` when it is
   stale or missing (there is no current plan to show).
